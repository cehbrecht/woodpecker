...
def test_list_fixes_contains_known_codes(cli_runner):
    result = cli_runner.invoke(cli, ["list-fixes", "--format", "text"])
...
def test_io_status_text_output_contains_expected_keys(cli_runner):
    result = cli_runner.invoke(cli, ["io-status"])
...
def test_io_status_json_output_structure(cli_runner):
    result = cli_runner.invoke(cli, ["io-status", "--format", "json"])
